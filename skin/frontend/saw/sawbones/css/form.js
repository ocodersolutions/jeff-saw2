...
// 			and therefore the liability falls on the person who has taken it upon himself to use this script
	...
	jQuery(document).ajaxStart(function(){
		jQuery('#preloader').delay(100).fadeIn(350);
		jQuery('#preloader').removeClass('none');
	});

	jQuery(document).ajaxStop(function(){
		jQuery('#preloader').fadeOut(300);
		jQuery('#preloader').addClass('none');
	});
	...
	multi_step_form = function(defaults)
	{
		...
		total_sections = jQuery(section_container+' '+section).length;

		// set the title of the section
		jQuery(section_container+" "+section_title).html(jQuery(section).attr('section'));
		
		// load the contents of the previous section
		jQuery(section_container+" ."+back_button).click(function() {
			// reset the errors
			error=0;
		...
		jQuery(section_container+" ."+next_button).click(function() {
			// reset the errors
			error=0;
		...
		jQuery(section_container).on("click", "."+submit_class,function() {
			//post_data(jQuery(post_type_class).html());
			//alert(jQuery(".post-type").val());
			post_data(section_container, jQuery(".post-type").val(), fields);
		});	
		
		// validate the input control's data 
		jQuery(section+' input, '+section+' select, '+section+' textarea').focusout(function() {
			Validate_Section_e(jQuery(this));
		});
		...
		jQuery(section).each(function(i){
			...
			if(button==back_button && currentIndex==i && currentIndex>0){
				if(currentIndex<total_sections){
					jQuery(section_container+" .submit").addClass("next");
					jQuery(section_container+" .submit").html("Next");
					jQuery(section_container+" .submit").removeClass("submit");
				}
				jQuery(this).slideUp();
			...
			if(button==back_button && currentIndex-1==i && currentIndex>0){
				jQuery(section_container+" "+section_title).html(jQuery(this).attr('section'));
	...
	function Validate_Section_e(sectionObject){
		// loop through the input controls in the current section
		//jQuery(sectionObject+' input, '+sectionObject+' select, '+sectionObject+' textarea').each(
			//function(index){  
				// create an object of the current input control
	...
	function ProgressBar(){
		progrssBarWidth = jQuery('.progress-bar').width(); // get the current width of the progress bar
		...
		jQuery('.progress').css('width',progressWidth+'%'); // set the width of the progress completed so far
		jQuery('.progress-text').html(progressWidth+'% Complete'); // set the percentage completed so far
	}

	function LoadFinalSection(sectionObject){
		if(complete_sections==total_sections-1){
			jQuery(sectionObject).addClass('current');
			...
			// load the next form section and hide the current one
			jQuery(section+':not('+sectionObject+')').each(function(){
				sectionobj = jQuery(this);
				...
				jQuery(sectionobj).find(' input, select, textarea').each(
					function(index){  
			...
			jQuery(sectionObject).html(confirmed_values);
			jQuery(section_container+" ."+next_button).html(submit_button);
			jQuery(section_container+" ."+next_button).addClass("submit");
			jQuery(section_container+" ."+next_button).removeClass("next");
		}
	}
	...
	function post_data(section_container, type, fields){
		if(type == "Database"){
			...
			success: function(data) {
				if(data){
					jQuery(section_container).html(data);
				}
			}
			});
		}
